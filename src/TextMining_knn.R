...
for (low in c(70, 100, 120))
{
  for (high in c(700,1000,1100))
  {
    ...
    for(k in ks)
    {
      ...
      for (i in c(1:folderCnt))
      {
        print(paste("Begin: lower", low, "upper", high, "k", k, "cross folder", i))
        ...
        #KNN
        data.knn.lsa <- knn(train=train.lsa.features, test=test.lsa.features, cl=train.lsa.classes, k=k)
        tb.lsa <- data.frame(pred = data.knn.lsa, actual = test.lsa.classes)
        data.knn.old <- knn(train=train.old.features, test=test.old.features, cl=train.old.classes, k=k)
        tb.old <- data.frame(pred = data.knn.old, actual = test.old.classes)
    ...
    result.lsa.tmp <- data.frame(lower=low, upper=high, k=ks, error=err_rates.lsa, err_var = err_var.lsa, precision_tp1.lsa = precisions.lsa, recall_tp1.lsa = recalls.lsa, f_score_tp1.lsa = 2/(1/precisions.lsa + 1/recalls.lsa))
    result.old.tmp <- data.frame(lower=low, upper=high, k=ks, error=err_rates.old, err_var = err_var.old, precision_tp1.old = precisions.old, recall_tp1.old = recalls.old, f_score_tp1.old = 2/(1/precisions.old + 1/recalls.old))
