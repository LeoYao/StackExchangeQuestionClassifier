...
for (low in c(50, 100, 150))
{
  for (high in c(700, 1000, 1100))
  {
    ...
    for (s in ss)
    {
      ...
      for (i in c(1:folderCnt))
      {
        print(paste("Begin: lower", low, "upper", high, "size", s, "cross folder", i))
        ...
        set.seed(5678)
        data.ann.lsa <- nnet(as.factor(TOPIC) ~ ., data = train.lsa, size = s, rang = 0.05, decay = 0, maxit = 100, MaxNWts = 10000)
        pred_value.lsa <- predict(data.ann.lsa, test.lsa.features, type='class')
        tb.lsa <- data.frame(pred = pred_value.lsa, actual = test.lsa.classes)
        set.seed(5678)
        data.ann.old <- nnet(as.factor(TOPIC) ~ ., data = train.old, size = s, rang = 0.05, decay = 0, maxit = 100, MaxNWts = 10000)
        pred_value.old <- predict(data.ann.old, test.old.features, type='class')
        tb.old <- data.frame(pred = pred_value.old, actual = test.old.classes)
        
        
        if (length(levels(tb.lsa$pred)) < 10)
        {
          levs <- c()
          for (lev in as.character(c(1:10)))
          {
            if (!(lev %in% levels(tb.lsa$pred)))
            {
              levs <- c(levs, lev)
            }
          }
          levels(tb.lsa$pred) <- c(levels(tb.lsa$pred), levs)
        }
        
        if (length(levels(tb.lsa$actual)) < 10)
        {
          levs <- c()
          for (lev in as.character(c(1:10)))
          {
            if (!(lev %in% levels(tb.lsa$actual)))
            {
              levs <- c(levs, lev)
            }
          }
          levels(tb.lsa$actual) <- c(levels(tb.lsa$actual), levs)
        }
        
        if (length(levels(tb.old$pred)) < 10)
        {
          levs <- c()
          for (lev in as.character(c(1:10)))
          {
            if (!(lev %in% levels(tb.old$pred)))
            {
              levs <- c(levs, lev)
            }
          }
          levels(tb.old$pred) <- c(levels(tb.old$pred), levs)
        }
        
        if (length(levels(tb.old$actual)) < 10)
        {
          levs <- c()
          for (lev in as.character(c(1:10)))
          {
            if (!(lev %in% levels(tb.old$actual)))
            {
              levs <- c(levs, lev)
            }
          }
          levels(tb.old$actual) <- c(levels(tb.old$actual), levs)
        }
    ...
    result.lsa.tmp <- data.frame(lower=low, upper=high, size = ss, error=err_rates.lsa, err_var = err_var.lsa, precision_tp1.lsa = precisions.lsa, recall_tp1.lsa = recalls.lsa, f_score_tp1.lsa = 2/(1/precisions.lsa + 1/recalls.lsa))
    result.old.tmp <- data.frame(lower=low, upper=high, size = ss, error=err_rates.old, err_var = err_var.old, precision_tp1.old = precisions.old, recall_tp1.old = recalls.old, f_score_tp1.old = 2/(1/precisions.old + 1/recalls.old))
