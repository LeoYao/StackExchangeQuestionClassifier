  ...
  for (high in c(700,1000,1100))
  {
    ...
    
    {
      ...
      for (i in c(1:folderCnt))
      {
        ...
        #NB
        data.nb.lsa<-naiveBayes(train.lsa.features, as.factor(train.lsa.classes))
        pred_value.lsa <- predict(data.nb.lsa, test.lsa.features, type='class')
        tb.lsa <- data.frame(pred = pred_value.lsa, actual = test.lsa.classes)
        data.nb.old<-naiveBayes(train.old.features, as.factor(train.old.classes))
        pred_value.old <- predict(data.nb.old, test.old.features, type='class')
      ...
      precisions.lsa <- c(precisions.lsa, prec_tmp.old/(10*folderCnt))
      recalls.lsa <- c(recalls.lsa, recal_tmp.lsa/(10*folderCnt))
      err_var.lsa <- c(err_var.lsa, var(err_arr.lsa, err_arr.lsa))
      
      err_rates.old <- c(err_rates.old, errorCnt.old/testCnt.old)
      precisions.old <- c(precisions.old, prec_tmp.old/(10*folderCnt))
      recalls.old <- c(recalls.old, recal_tmp.old/(10*folderCnt))
