  ...
  for (high in c(700,1000,1100))
  {
    ...
    
    {
      ...
      for (i in c(1:folderCnt))
      {
        ...
        #Decision Tree
        data.dt.lsa <- rpart(as.factor(TOPIC) ~ ., data=train.lsa, method="class", parms=list(split='gini'), control=rpart.control(minsplit=1,minbucket=1, cp=0))
        pred_value.lsa <- predict(data.dt.lsa, newdata=test.lsa.features, type="class")
        tb.lsa <- data.frame(pred = pred_value.lsa, actual = test.lsa.classes)
        data.dt.old <- rpart(as.factor(TOPIC) ~ ., data=train.old, method="class", parms=list(split='gini'), control=rpart.control(minsplit=1,minbucket=1, cp=0))
        pred_value.old <- predict(data.dt.old, newdata=test.old.features, type="class")
